...
export const startTicketWorker = () => {
  const worker = new Worker(
    ...
    async (job) => {
      ...
      try {
        ...
        await FileProcess.updateOne({ processId }, { $inc: { processed: 1 } });

        return true;
      } catch (error) {
        logger.error({ error, processId }, 'Job failed');

        await FileProcess.updateOne({ processId }, { $inc: { failed: 1 } });
  ...
  worker.on('completed', async (job) => {
    ...
    if (process.processed + process.failed >= process.total) {
      process.status = process.failed > 0 ? 'completed_with_errors' : 'completed';
  ...
  worker.on('failed', async (job, err) => {
    logger.error({ err, jobId: job?.id }, 'Worker job failed permanently');
  });
